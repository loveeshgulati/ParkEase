...
using ParkEase.Booking.BackgroundServices;
using ParkEase.Booking.Consumers;
using ParkEase.Booking.Data;
using ParkEase.Booking.Interfaces;
using ParkEase.Booking.Middleware;
using ParkEase.Booking.Repositories;
...
Log.Logger = new LoggerConfiguration()
    ...
    .WriteTo.Console()
    .WriteTo.File("logs/parkease-booking-.log", rollingInterval: RollingInterval.Day)
    .CreateLogger();
builder.Host.UseSerilog();

// ── PostgreSQL + EF Core ──────────────────────────────────────────────────────
builder.Services.AddDbContext<BookingDbContext>(options =>
    options.UseNpgsql(builder.Configuration.GetConnectionString("DefaultConnection"), x => x.MigrationsHistoryTable("__EFMigrationsHistory_Booking")));
...
builder.Services.AddSwaggerGen(c =>
{
    c.SwaggerDoc("v1", new OpenApiInfo
    {
        ...
        Version = "v1",
        Description = "Booking lifecycle management microservice for ParkEase"
    });
